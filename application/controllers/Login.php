...
class Login extends CI_Controller {
    ...
    public function index() {
        $this->prepare_view_login_data();
    }

    private function prepare_view_login_data() {
        $data = [
            'title' => 'Home',
            'js' => [
                'main/login.js'
        ...
        return $this->template->load('main/layout', 'main/login', $data);
    }
    ...
    public function check_credentials_emp() {
        if (!$this->validate_input_emp()) {
            ...
            if ($check) {
                ...
                $this->session->set_flashdata('error', 'Invalid NPP or password');
                $this->session->set_flashdata('nrik', $data['nrik']);
    ...
    private function process_user_login_emp($user) {
        if ($user->status == INACTIVE) {
            $this->session->set_flashdata('error', 'The user is inactive');
            redirect('login');
        } else {
            $data = [
                'agent' => get_browser_agent(),
    ...
    private function validate_input_emp() {
        $this->form_validation->set_rules('nrik', 'NPP', 'required|trim|min_length[4]|max_length[4]');
        $this->form_validation->set_rules('password', 'Password', 'required|trim');
        $this->form_validation->set_error_delimiters('', '');
        return $this->form_validation->run();
    }
