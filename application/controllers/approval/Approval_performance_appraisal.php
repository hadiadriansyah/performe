...
class Approval_performance_appraisal extends Base_controller {

    protected $repository;

    public function __construct() {
        parent::__construct();
        $this->load->repository('approval/Approval_performance_appraisal_repository');
        $this->repository = new Approval_performance_appraisal_repository();
    ...
    public function index() {
        $data = array_merge($this->global, [
            ...
            'js' => [
                ...
                'admin/approval/approval-pa/approval-pa.js',
                'admin/approval/approval-pa/kpi-list.js',
                'admin/approval/approval-pa/target-actual.js',
                'admin/approval/approval-pa/submit.js',
            ],
        ]);
        $this->template->load('admin/layout', 'admin/approval/approval_pa/index', $data);
    }
    ...
    public function add_edit_pa() {
        $pa_id = $this->input->post('pa_id');
        if (!$this->validate_input_add_edit_pa()) {
            $this->handle_validation_error_add_edit_pa();
            return;
        }

        $data = $this->collect_input_add_edit_pa();
        if (!$pa_id) {
            $this->save_pa($data);
        } else {
            $this->update_pa($data);
    ...
    private function validate_input_add_edit_pa() {
        return true;
    }

    private function handle_validation_error_add_edit_pa() {
        $errors = $this->collect_form_errors_add_edit_pa();
        $this->json_response->error('Failed to collect data.', $errors);
    }

    private function collect_form_errors_add_edit_pa() {
        return [];
    }

    private function collect_input_add_edit_pa() {
        $data_pa = [
        ...
        if (empty($this->input->post('pa_id'))) {
            ...
            $input_data['id'] = $this->input->post('pa_id');
            if ($this->global['vendor_id']) {
                $input_data['updated_by'] = $this->is_admin ? $this->global['vendor_id'] : $this->global['employee_id'];
            }
        }
    ...
    private function save_pa($data) {
        $store = $this->repository->store_pa($data);
        $message = $store ? 'Data Successfully Saved.' : 'Failed to save data.';
        
        $this->json_response->{$store ? 'success' : 'error'}($message, $store);
    }

    private function update_pa($data) {
        $store = $this->repository->update_pa($data);
        $message = $store ? 'Data Successfully Updated.' : 'Failed to update data.';
    ...
    public function submit_pa() {
        ...
        if (!$this->validate_input_submit_pa()) {
            $this->handle_validation_error_submit_pa();
            return;
        }

        $this->db->trans_start();

        $data_submit = $this->collect_input_submit_pa($id);
        $data_hist = $this->collect_input_hist_approval_pa($id);

        $this->repository->update_submit_pa($data_submit);
        $this->repository->store_hist_approval_pa($data_hist);

        $this->db->trans_complete();

        if ($this->db->trans_status() === FALSE) {
            $this->json_response->error('Submit PA Failed');
        } else {
            $this->json_response->success('Submit PA Success');
        }

    }

    private function validate_input_submit_pa() {
    ...
    private function collect_input_submit_pa($id) {
    ...
    private function collect_input_hist_approval_pa($id) {
        $input_data = [
            'approval_pa_id' => $id,
            'puk_number' => $this->input->post('puk_number'),
    ...
    private function handle_validation_error_submit_pa() {
        $errors = $this->collect_form_errors_submit_pa();
        $this->json_response->error('Failed to save data.', $errors);
    }

    private function collect_form_errors_submit_pa() {
        return [
    ...
    // private function save_hist_approval_pa($data) {
    //     $store = $this->repository->save_hist_approval_pa($data);
    //     $message = $store ? 'Data Successfully Submitted.' : 'Failed to submit data.';
    ...
    public function get_approval_by_approval_pa() {
    ...
    public function get_pa_by_id($id) {
        ...
        $data = $this->repository->get_pa_by_id($id);
    ...
    public function get_hist_approval_pa() {
        $approval_pa_id = $this->input->post('approval_pa_id');
        $approval = $this->repository->get_hist_approval_pa($approval_pa_id);
        $message = $approval ? 'Data successfully fetched.' : 'Failed to fetch data.';
        $this->json_response->{$approval ? 'success' : 'error'}($message, $approval);
    }
