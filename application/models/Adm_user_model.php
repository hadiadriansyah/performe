...
class Adm_user_model extends CI_Model {
    ...
    public function check_credentials($nrik, $password) {
        $this->db->select("{$this->table}.id, {$this->table}.nama_login, {$this->table}.id_peg, {$this->table}.status, emp_data_peg.nama as emp_name, emp_data_peg.status_peg, emp_data_peg.foto, hist_jabatan.id_jabatan, hist_jabatan.id_unit_kerja");
        ...
        if ($query->num_rows() === 1) {
            $hashed_password = $this->db->select("password")->from($this->table)->where("nama_login", $nrik)->get()->row()->password;
