...
class Approval_target_model extends CI_Model {
    protected $table_approval_target = 'npm_approval_target';
    ...
    //     $this->db->from($this->table_approval_target)
    //                 ->where("{$this->table_approval_target}.pa_individual_id", $pa_individual_id)
    //                 ->where("{$this->table_approval_target}.month_period", $month_period);
    //     $result = $this->db->get();
    ...
    public function get_by_pa_individual_id($pa_individual_id, $month_period) {
        $sql = "
            ...
            FROM
                {$this->table_approval_target} at
            LEFT JOIN
                LatestApprovalData lad ON at.id = lad.approval_target_id
            LEFT JOIN
    ...
    public function get_approval_employees($year_period_id, $placement_unit_id, $position_id) {
        $this->db->select("{$this->table_approval_target}.*, {$this->table_emp_data_peg}.nama as employee_name, {$this->table_emp_data_peg}.nrik as employee_npp, {$this->table_pa_individuals}.position_id, {$this->table_pa_individuals}.placement_unit_id, {$this->table_pa_individuals}.from_month, {$this->table_pa_individuals}.to_month, {$this->table_md_jabatan}.nm_jabatan as position_name, muku.nm_unit_kerja as unit_name, mukp.nm_unit_kerja as placement_unit_name")
                    ...
                    ->group_start()
                        ->where("{$this->table_approval_target}.puk_1_unit", $placement_unit_id)
                        ->where("{$this->table_approval_target}.puk_1_position", $position_id)
                    ->group_end()
                    ->or_group_start()
                        ->where("{$this->table_approval_target}.puk_2_unit", $placement_unit_id)
                        ->where("{$this->table_approval_target}.puk_2_position", $position_id)
                    ->group_end()
                    ->order_by("{$this->table_approval_target}.puk_1_status", "ASC")
                    ->order_by("{$this->table_approval_target}.puk_2_status", "ASC");
    ...
    public function exists(array $data) {
        $this->db->from($this->table_approval_target)
                    ->where("{$this->table_approval_target}.pa_individual_id", $data['pa_individual_id'])
                    ->where("{$this->table_approval_target}.month_period", $data['month_period'])
                    ->where("{$this->table_approval_target}.month_periods", $data['month_periods']);
        $result = $this->db->get();
        return [
    ...
    public function store($data) {
        return $this->db->insert($this->table_approval_target, $data);
    }

    public function update($data) {
        return $this->db->update($this->table_approval_target, $data, [
            'id' => $data['id'],
