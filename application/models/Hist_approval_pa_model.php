...
class Hist_approval_pa_model extends CI_Model {
    protected $table_hist_approval_pa = 'npm_hist_approval_pa';

    public function get_by_approval_pa_id($approval_pa_id) {
        $sql = "
            WITH RankedData AS (
                SELECT
                    *,
        ...
        $query = $this->db->query($sql, array($approval_pa_id));
        return $query->row_array();
    }

    public function store($data) {
        return $this->db->insert($this->table_hist_approval_pa, $data);
    }
