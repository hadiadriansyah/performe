...
class Kpi_counter_model extends CI_Model {
    protected $table = 'npm_kpi_counters';
    protected $column_order = [null, 'counter', null, 'npm_year_periods.year_period', 'created_at'];
    protected $column_search = ['counter'];
    protected $order = ['npm_year_periods.year_period' => 'desc', 'counter' => 'asc'];
    ...
    public function exists(array $data) {
        $this->db->select("{$this->table}.counter, npm_year_periods.year_period")
                    ->from($this->table)
                    ->join('npm_year_periods', "{$this->table}.year_period_id = npm_year_periods.id", 'left')
                    ->where("LOWER({$this->table}.counter)", strtolower($data['counter']))
    ...
    public function get_options_by_year_period_id($search, $page, $year_period_id) {
        ...
        if ($year_period_id) {
            $this->db->select("{$this->table}.id, {$this->table}.counter")
                        ->like("LOWER(CAST(counter AS TEXT))", $search)
            ...
            $total_count = $this->db->like("LOWER(CAST(counter AS TEXT))", $search)->where('year_period_id', $year_period_id)->count_all_results($this->table);
        }
