...
class Kpi_polarization_model extends CI_Model {
    ...
    public function exists(array $data) {
        $this->db->select("{$this->table}.polarization, npm_year_periods.year_period")
                    ->from($this->table)
                    ->join('npm_year_periods', "{$this->table}.year_period_id = npm_year_periods.id", 'left')
    ...
    public function get_options_by_year_period_id($search, $page, $year_period_id) {
        ...
        if ($year_period_id) {
            $this->db->select("{$this->table}.id, {$this->table}.polarization")
                        ->like("LOWER(CAST(polarization AS TEXT))", $search)
            ...
            $total_count = $this->db->like("LOWER(CAST(polarization AS TEXT))", $search)->where('year_period_id', $year_period_id)->count_all_results($this->table);
        }
