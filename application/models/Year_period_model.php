...
class Year_period_model extends CI_Model {
    ...
    public function exists(array $data) {
        $this->db->select("{$this->table}.year_period")
                    ->from($this->table)
    ...
    public function get_options($search = '', $page = 1) {
        ...
        $this->db->select("{$this->table}.id, {$this->table}.year_period")
                    ->where("year_period ILIKE", "%{$search}%")
                    ->limit(10, ($page - 1) * 10);
        $query = $this->db->get($this->table);

        $total_count = $this->db->where("year_period ILIKE", "%{$search}%")->count_all_results($this->table);
