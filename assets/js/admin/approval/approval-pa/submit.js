...
const KTSubmitPa = (() => {
    ...
    const handleFormSubmission = async () => {
        ...
        try {
            ...
            const formDetails = {
                url: 'approval/approval_performance_appraisal/submit_pa',
    ...
    const setupPaSubmitApproval = async (data) => {
    ...
    const getApproval = async () => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_performance_appraisal/get_approval_by_approval_pa`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    const getApprovalSubmitPa = async () => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_performance_appraisal/get_approval_pa`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({
                    pa_individual_id: KTApprovalPa.getApprovalEmployeeData().pa_individual_id
                }).toString()
            });
    ...
    const updateApprovalStatus = (status, selector, data) => {
        const submitPaButton = document.querySelector('[data-kt-submit-pa-button-action="submit_puk"]');
        if (submitPaButton) {
            
            pukNumber = submitPaButton.getAttribute('data-puk-number');
            if (pukNumber == 1) {
                submitPaButton.disabled = data.puk_1_status == 2 || data.puk_1_status == 3;

            } else {
                submitPaButton.disabled = data.puk_2_status == 2 || data.puk_2_status == 3;
            }
        }
        const statusMap = {
            1: 'Submitted',
    ...
    const displayApprovalData = async () => {
        const formApprovalPa = document.querySelector('#kt_approval_pa_form');
            ...
            approvalData.approval_1.forEach((approver, index) => {
                if (index === 0) {
                    html += `<tr>
                                ...
                                <td rowspan="${approvalData.approval_1.length}" class="text-center">
                                    ${approver.unit_approval_1_id === placementUnitId && approver.position_approval_1_id === positionId ? '<button class="btn btn-sm btn-light-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_submit_pa" data-kt-submit-pa-button-action="submit_puk" data-puk-number="1" disabled>Submit</button>' : ''}
                                </td>
                            </tr>`;
                } else {
                    html += `<tr>
            ...
            approvalData.approval_2.forEach((approver, index) => {
                if (index === 0) {
                    html += `<tr>
                                ...
                                <td rowspan="${approvalData.approval_2.length}" class="text-center">
                                    ${approver.unit_approval_2_id === placementUnitId && approver.position_approval_2_id === positionId ? '<button class="btn btn-sm btn-light-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_submit_pa" data-kt-submit-pa-button-action="submit_puk" data-puk-number="2" disabled>Submit</button>' : ''}
                                </td>
                            </tr>`;
                } else {
                    html += `<tr>
    ...
    return {
        init: function () {
            element = document.getElementById('kt_modal_submit_pa');

            form = element.querySelector('#kt_modal_submit_pa_form');
            approvalContainer = document.querySelector('#kt_table_pa_submit_approval');
            submitButton = form.querySelector('[data-kt-submit-pa-modal-action="submit"]');
            cancelButton = form.querySelector('[data-kt-submit-pa-modal-action="cancel"]');
            closeButton = element.querySelector('[data-kt-submit-pa-modal-action="close"]');
            initSubmitTarget();
            ...
            //     const submitPaButton = event.target.closest('[data-kt-submit-pa-button-action="submit_puk"]');
            //     if (submitPaButton) {
            //         pukNumber = submitPaButton.getAttribute('data-puk-number');
            //     }
            // });
