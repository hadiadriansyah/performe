...
const KTApprovalTarget = (() => {
    let formApprovalTarget;
    let approvalEmployeeContainer;
    let kpiContainer;
    let targetSubmitContainer;

    let targetYearPeriodId;
    ...
    const initOptionSelect2 = () => {
        $('[name="employee_id"]').select2({
            ajax: {
                url: `${siteUrl}approval/approval_target/get_employee_options`,
                dataType: 'json',
                delay: 250,
                data: params => ({
                    q: params.term || '',
            ...
            const employeeId = formApprovalTarget.querySelector('[name="employee_id"]').value;
            const npp = formApprovalTarget.querySelector('[name="npp"]');
            const positionSelect = formApprovalTarget.querySelector('[name="position_id"]');
            const description = formApprovalTarget.querySelector('#description');
            const unitSelect = formApprovalTarget.querySelector('[name="unit_id"]');
            const placementUnitSelect = formApprovalTarget.querySelector('[name="placement_unit_id"]');
        ...
        $('[name="year_period_id"]').select2({
            ajax: {
                url: `${siteUrl}approval/approval_target/get_year_period_options`,
                dataType: 'json',
                delay: 250,
                data: params => ({
                    q: params.term || '',
    ...
    const getPositionUnitPlacementUnitByEmployeeId = async (employeeId) => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_position_unit_placement_unit_by_employee_id/${employeeId}`);
    ...
    const initEmployee = async () => {
        ...
        if (employee) {
            const employeeSelect = formApprovalTarget.querySelector('[name="employee_id"]');
    ...
    const getEmployee = async () => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_employee`, { method: 'GET' });
            const result = await response.json();
            return result.data;
        } catch (error) {
            return false;
    ...
    const setupApprovalEmployee = async () => {
        ...
        try {
            const yearPeriodId = formApprovalTarget.querySelector('[name="year_period_id"]').value;
            const unitId = formApprovalTarget.querySelector('[name="unit_id"]').value;
    ...
    const getApprovalEmployees = async (yearPeriodId, unitId, placementUnitId, positionId) => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_approval_employees`, {
                method: 'POST',
    ...
    return {
        init: function () {
            formApprovalTarget = document.querySelector('#kt_approval_target_form');
            ...
            formApprovalTarget.addEventListener('reset', () => {
                const selects = formApprovalTarget.querySelectorAll('select');
        ...
        getTargetYearPeriodId: function () {
            return targetYearPeriodId;
        },
        getApprovalTargetId: function () {
