...
const KTKpiIndividualList = (() => {
    ...
    const initOptionSelect2 = (row) => {
        const yearPeriodId = KTApprovalTarget.getTargetYearPeriodId();
        $(`#kpi_id_${row.id}`).select2({
            ajax: {
                url: `${siteUrl}approval/approval_target/get_kpi_options_by_year_period_id`,
                dataType: 'json',
                delay: 250,
                data: params => ({
                    q: params.term || '',
            ...
            if (kpi) {
                ...
                const paIndividualId = KTApprovalTarget.getApprovalEmployeeData().pa_individual_id;
                const yearPeriodId = KTApprovalTarget.getTargetYearPeriodId();
    ...
    async function storeUpdateKpi(data, isSave = false) {
        ...
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/store_update_kpi`, {
                method: 'POST',
    ...
    const setupKpiData = async () => {
        const approvalEmployeeData = KTApprovalTarget.getApprovalEmployeeData() ?? {};
        ...
        kpiData = [];
        const kpiIndividual = await getKpiIndividualByPaId(approvalEmployeeData.pa_individual_id);
        if (kpiIndividual) {
        ...
        renderKpi();
        await KTSubmitTarget.setupTargetSubmitApproval(approvalEmployeeData);
    }
    ...
    async function getKpiById(id) {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_kpi_by_id/${id}`, { method: 'GET' });
            const result = await response.json();
    ...
    const getKpiIndividualTargetByPaIndividualIdYearPeriodId = async () => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_kpi_individual_target_by_pa_individual_id_year_period_id`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({
                    pa_individual_id: KTApprovalTarget.getApprovalEmployeeData().pa_individual_id,
                    year_period_id: KTApprovalTarget.getTargetYearPeriodId(),
                }).toString()
            });
            const result = await response.json();
            return result.data;
    ...
    const getKpiIndividualByPaId = async (paId) => {
        try {
            const response = await fetch(`${siteUrl}approval/approval_target/get_kpi_individual_by_pa_id/${paId}`);
            await new Promise(resolve => setTimeout(resolve, 300));
            const result = await response.json();
            return result.data;
        } catch (error) {
     ...
     const handleDeleteRow = async (id, selectedText) => {
        try {
            ...
            if (kpiDataItem && kpiDataItem.mode == 'add') {
                ...
                const response = await fetch(`${siteUrl}approval/approval_target/delete_kpi`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
