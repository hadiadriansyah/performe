...
const KTSubmitTarget = (() => {
    ...
    const handleSubmitTarget = async () => {
        ...
        try {
            ...
            const formDetails = {
                url: 'goals_settings/kpi_individual/submit_target',
    ...
    const setupTargetSubmitApproval = async (gsData) => {
    ...
    const getApproval = async () => {
        try {
            const response = await fetch(`${siteUrl}goals_settings/kpi_individual/get_approval_by_goals_settings`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    const getApprovalSubmitTarget = async () => {
        try {
            const response = await fetch(`${siteUrl}goals_settings/kpi_individual/get_approval_target`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    const updateSubmitStatus = (status, selector) => {
        ...
        if (status != 0) {
            ...
            document.querySelectorAll('[data-kt-kpi-table-filter="delete_row"]').forEach(element => {
                element.classList.add('d-none');
            });
    ...
    return {
        init: function () {
            element = document.getElementById('kt_table_target_submit');
            form = element.querySelector('#kt_target_submit_form');
            approvalContainer = document.querySelector('#kt_table_target_submit_approval');

            submitButton = form.querySelector('[data-kt-target-submit-button-action="submit"]');
            addKpiButton = document.querySelector('[data-kt-kpis-button-action="add_kpi"]');
