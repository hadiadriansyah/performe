...
const KTKpisList = (function () {
    let datatable;
    let table;
    let dataCheckboxes = [];

    const initKpiData = function () {
        datatable = $("#kt_table_kpis").DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ...
            ajax: {
                url: `${siteUrl}master/kpi/data_server`,
                type: "POST",
                data: function (d) {
                    d.year_period_id = $('[data-kt-kpi-table-filter="year_period_id"]').val();
                }
            },
            columns: [
                { data: 'id' },
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return `
                            <div class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2" data-bs-dismiss="click" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Edit KPI" data-kt-kpi-table-filter="edit_row" data-id="${row.id}" id="kt_modal_view_event_edit">
                            ...
                            <div class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-dismiss="click" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="Delete KPI" data-id="${row.id}" data-kt-kpi-table-filter="delete_row" id="kt_modal_view_event_delete">
                                <span class="svg-icon svg-icon-2">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    const handleSearchDatatable = function () {
        const filterSearch = document.querySelector('[data-kt-kpi-table-filter="search"]');
    ...
    const handleFilterDatatable = function () {
        const filterButton = document.querySelector('[data-kt-kpi-table-filter="filter"]');
    ...
    const handleResetForm = function () {
        const resetButton = document.querySelector('[data-kt-kpi-table-filter="reset"]');
        resetButton.addEventListener('click', function () {
            const filterForm = document.querySelector('[data-kt-kpi-table-filter="form"]');
    ...
    const handleDeleteSelected = async (dataCheckboxes) => {
        ...
        for (let i = 0; i < total; i++) {
            try {
                const response = await fetch(`${siteUrl}master/kpi/delete`, {
                    method: 'POST',
    ...
    const handleDeleteRows = function () {
        table.addEventListener('click', function (e) {
            if (e.target.closest('[data-kt-kpi-table-filter="delete_row"]')) {
                ...
                const kpiText = parent.querySelectorAll('td')[1].innerText;
                const id = e.target.closest('[data-kt-kpi-table-filter="delete_row"]').getAttribute('data-id');

                Swal.fire({
                    text: "Are you sure you want to delete " + kpiText + "?",
                    icon: "warning",
                    showCancelButton: true,
                    buttonsStyling: false,
                    confirmButtonText: "Yes, delete!",
                    ...
                    if (result.value) {
                        Swal.fire({
                            text: "Deleting " + kpiText,
                            icon: "info",
                            buttonsStyling: false,
                            showConfirmButton: false,
                            didOpen: () => {
                                Swal.showLoading();
                                handleDeleteRow(id, kpiText);
                            }
                        });
                    } else if (result.dismiss === 'cancel') {
                        KTAlertDialog.showErrorMessage(kpiText + " was not deleted.");
                    }
                });
            }
        });
    ...
    const handleDeleteRow = async (id, kpiText) => {
        try {
            const response = await fetch(`${siteUrl}master/kpi/delete`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            ...
            await new Promise(resolve => setTimeout(resolve, 300));
            const result = await response.json();
            handleDeleteResponse(result, kpiText);
        } catch (error) {
            KTAlertDialog.showErrorMessage("Data could not be deleted. The record is still related to other records.");
        }
    };    

    const handleDeleteResponse = (response, kpiText) => {
        if (response.status === 'success') {
            KTAlertDialog.showSuccessMessage(kpiText + " was deleted.", () => {
                datatable.draw();
            });
        } else {
