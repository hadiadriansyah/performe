...
const KTIndividualPerformanceAppraisal = (() => {
    let formIndividualPerformanceAppraisal;
    ...
    const initOptionSelect2 = () => {
        $('[name="employee_id"]').select2({
            ajax: {
                url: `${siteUrl}performance_appraisal/individual_performance_appraisal/get_employee_options`,
                dataType: 'json',
                delay: 250,
                data: params => ({
                    q: params.term || '',
            ...
            setupGoalsSettings();
            const employeeId = formIndividualPerformanceAppraisal.querySelector('[name="employee_id"]').value;
            const npp = formIndividualPerformanceAppraisal.querySelector('[name="npp"]');
            const positionSelect = formIndividualPerformanceAppraisal.querySelector('[name="position_id"]');
            const description = formIndividualPerformanceAppraisal.querySelector('#description');
            const unitSelect = formIndividualPerformanceAppraisal.querySelector('[name="unit_id"]');
            const placementUnitSelect = formIndividualPerformanceAppraisal.querySelector('[name="placement_unit_id"]');
        ...
        $('[name="year_period_id"]').select2({
            ajax: {
                url: `${siteUrl}performance_appraisal/individual_performance_appraisal/get_year_period_options`,
                dataType: 'json',
                delay: 250,
                data: params => ({
                    q: params.term || '',
    ...
    const getPositionUnitPlacementUnitByEmployeeId = async (employeeId) => {
        try {
            const response = await fetch(`${siteUrl}performance_appraisal/individual_performance_appraisal/get_position_unit_placement_unit_by_employee_id/${employeeId}`);
    ...
    const initEmployee = async () => {
        ...
        if (employee) {
            const employeeSelect = formIndividualPerformanceAppraisal.querySelector('[name="employee_id"]');
    ...
    const getEmployee = async () => {
        try {
            const response = await fetch(`${siteUrl}performance_appraisal/individual_performance_appraisal/get_employee`, { method: 'GET' });
            const result = await response.json();
            return result.data;
        } catch (error) {
            return false;
    ...
    const setupGoalsSettings = async () => {
        ...
        const yearPeriodId = formIndividualPerformanceAppraisal.querySelector('[name="year_period_id"]').value;
        const employeeId = formIndividualPerformanceAppraisal.querySelector('[name="employee_id"]').value;
            ...
            if (goalsSettings) {
                goalsSettings.forEach((gs, index) => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="text-end">
                            <div class="btn btn-icon btn-sm btn-color-gray-4000 btn-active-icon-success me-2" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-dark" data-bs-placement="top" title="View" data-id="${gs.id}" data-kt-create-gs-table-filter="view_row">
    ...
    const getGoalsSettings = async (yearPeriodId, employeeId) => {
        try {
            const response = await fetch(`${siteUrl}performance_appraisal/individual_performance_appraisal/get_goals_settings`, {
                method: 'POST',
        ...
        viewButtons.forEach(button => {
            button.addEventListener('click', async function() {
                ...
                try {
                    ...
                    await KTIndividualPerformanceAppraisalList.setupKpiData(id);
                } catch (error) {
    ...
    return {
        init: function () {
            formIndividualPerformanceAppraisal = document.querySelector('#kt_kpi_individual_form');
            ...
            formIndividualPerformanceAppraisal.addEventListener('reset', () => {
                const selects = formIndividualPerformanceAppraisal.querySelectorAll('select');
...
KTUtil.onDOMContentLoaded(function () {
    KTIndividualPerformanceAppraisal.init();
});