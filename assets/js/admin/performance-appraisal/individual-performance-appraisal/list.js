...
const KTIndividualPerformanceAppraisalList = (() => {
    let kpiContainer;
    let submitButton;
    ...
    const setupKpiData = async (id) => {
        await initIndexScoreRule();
        const goalsSettingsData = KTIndividualPerformanceAppraisal.getGoalsSettingsData() ?? {};
        const gsData = goalsSettingsData[id];
        kpiContainer.querySelector('[name="position_name"]').value = gsData.position_name;

        firstMonthPeriod = gsData.from_month;
        lastMonthPeriod = gsData.to_month;
        ...
        kpiData = [];
        const kpiIndividual = await getIndividualPerformanceAppraisalByPaId(gsData.id);
        if (kpiIndividual) {
        ...
        renderKpi();
        await KTSubmitTarget.setupPaSubmitApproval(gsData);
    }
    ...
    async function getKpiById(id) {
        try {
            const response = await fetch(`${siteUrl}performance_appraisal/individual_performance_appraisal/get_kpi_by_id/${id}`, { method: 'GET' });
            const result = await response.json();
    ...
    const toggleSubmitButton = () => {
        const elementPaSubmit = document.getElementById('kt_table_pa_submit');
        const formPaSubmit = elementPaSubmit.querySelector('#kt_pa_submit_form');
        const paSubmitButton = formPaSubmit.querySelector('[data-kt-pa-submit-button-action="submit"]');
        if (isSubmit) {
            paSubmitButton.disabled = false;
            if (paSubmitButton.classList.contains('btn-secondary')) {
                paSubmitButton.classList.remove('btn-secondary');
            }
            if (!paSubmitButton.classList.contains('btn-primary-bs')) {
                paSubmitButton.classList.add('btn-primary-bs');
            }
        } else {
            paSubmitButton.disabled = true;
            if (paSubmitButton.classList.contains('btn-primary-bs')) {
                paSubmitButton.classList.remove('btn-primary-bs');
            }
            if (!paSubmitButton.classList.contains('btn-secondary')) {
                paSubmitButton.classList.add('btn-secondary');
            }
        }
    }
    ...
    const calculateDataPerformanceOnChange = async () => {
        ...
        try {
            kpiData.forEach(async kpi => {
                ...
                document.querySelector(`#index_${kpi.id}`).innerText = 0;
                document.querySelector(`#weight_${kpi.id}`).value = weightValue;
                document.querySelector(`#score_${kpi.id}`).innerText = 0;
                updateScore();
            });
...
KTUtil.onDOMContentLoaded(function () {
    KTIndividualPerformanceAppraisalList.init();
});